...
Here, the bases of each prism are parallel to each other. 
In this configuration, the beam exits from the second prism parallel to the incident beam.
...
While it might be difficult to measure $\theta_0$ accurately,
in most pragmatical situations, it can be close to the Brewster angle.
Thus, we assume that it is given as an input parameter.
And the apex angle $\alpha (=\theta_1 + \theta_2)$  is also given as an input parameter.
...
As
$$
\frac{dn}{d\Omega} = -\frac{\lambda_l^2}{2 \pi c} \frac{dn}{d\lambda}
$$ is computationally calculable by using Sellmeier equation, @eq-gdd-ultrafastphenomena can be computed.
...
&= \frac{1}{n^2} \left(\frac{\lambda_l^2}{2\pi c}\frac{dn}{d\lambda}\right) \\
\left( \frac{d}{d\Omega} \theta_1 \right)^2 &=
 \frac{1}{n^4} \frac{\lambda_l^4}{4\pi c^2} \left( \frac{dn}{d\lambda}\right)^2 
\end{aligned}
$$
...
&= \frac{-\lambda_l^3}{2 \pi c^2} 4L \left(\frac{dn}{d\lambda}\right)^2
\end{aligned}
$$
...
\begin{aligned}
&- \frac{n \omega_l}{c}L_g\left(\left.\frac{d\theta_1}{d\Omega}\right|_{\omega_l}\right)^2
= - \frac{L_g}{n^3} \frac{\lambda_l^3}{2 \pi c^2}  \left( \frac{dn}{d\lambda}\right)^2
\end{aligned}
$$
...
Internally, the following physical units are used:
...
* P_{O_2C}_mm
* L_mm
* alpha_deg (apex angle)
* theta_0_deg (incident angle, default: None (=Brewster angle)):
* material (default: SF11)
* wavelength_nm (central wavelength, default: 800 nm)
